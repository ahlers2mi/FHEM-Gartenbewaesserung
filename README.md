...
|---|---|---|
| **1.0.30** | 2026-06-09 | Neu: **Loop-Breaker** gegen endloses Nachfüll↔Leerlauf-Pendeln. Läuft das Fass während einer Bewässerung trotz wiederholtem Nachfüllen binnen Sekunden wieder leer (z.B. IBC leer **und** keine Hauswasser-Reserve), bricht das Modul nach `barrelEmptyMaxRefillAttempts` Versuchen (Default 3) ab → State `stopped - no water`, statt Pumpe/Ventile endlos zu takten. **Automatischer Neustart**, sobald wieder Wasser gemeldet wird: `barrelFull:yes`, `ibcEmpty:no` (IBC hat Wasser) oder `raining:yes`. Neues Attribut `barrelEmptyMaxRefillAttempts` (0 = aus = altes Verhalten). |
